...
下面我会使用预先构建的 `./example/echo.wasm` 来进行演示. 它最初是由 CPP 编写的, 然后使用 wasi-sdk 编译到 WebAssembly 代码. 大体上来说, 它和我们在 Linux 下常用的 `echo` 命令差不多, 其源代码为:
...
使用 WASC 编译 `echo.wasm` 文件, 这将得到一个本机可执行的二进制文件:

```sh
$ ./build/wasc --save ./example/echo.wasm
```

注意, `--save` 命令将保存编译过程中生成的临时文件, 这有助于我们对 WASC 的原理进行讲解. 来尝试下编译的成果, 我们调用 echo 可执行文件, 并尝试让它输出 "Hello World!".
...
让我们将先聚焦在 `./example/echo_build` 文件夹下, 它是编译过程中生成的副产物. 只有接收 `--save` 参数它们才会被保留下来.
...
WASC 编译的第一步是利用 WAVM 生成目标文件, 文件名 `echo.o`. 它是 WAVM 在 JIT 编译阶段的实际输出, 主要是机器代码, 同时包含允许链接程序查看其中包含哪些符号以及其正常工作所需的符号的信息. 可以使用 objdump 工具来分析一下这个文件, 并截取其中 SYMBOL TABLE 的一部分:
...
不错, 我们大概已经知道 WAVM 的运行时将要给它生成的 JIT 代码提供什么数据了. WASC 在这里做了一件事, 它仅仅使用极少量的代码(远远少于 WAVM), 为目标文件提供它缺少的东西. 这部分代码被称为 WASC 的胶水代码. 胶水代码由 C 编写而成.
...
```sh
$ gcc -g -w -o example/echo_build/echo example/echo_build/echo.o example/echo_build/echo.c example/echo_build/platform/posix_x86_64_wasi_runtime.S
```
...
首先将目光聚焦在 `./wasc_dapp_demo_ckb_vm/example/main.c` 文件中. 它是一个简单的 RISC-V 应用程序, 在程序中调用了一次 syscall, 然后将一个字符串的地址和长度作为参数之一传向 RISC-V 宿主平台. 我已经事先使用了 riscv64-unknown-elf-gcc 编译出了结果, 它位于 `./wasc_dapp_demo_ckb_vm/example/main` 路径.
...
我们准备执行这个 main 程序. 负责这个职责的是 ckb-vm, 它是一个使用 rust 编写的 RISC-V 虚拟机, 目前被应用在多个区块链上. 我们将使用 ckb-vm 来对这个 RISC-V 应用程序进行解释执行, 可以直接输入以下命令:

```sh
$ cargo run -- example/main
```

ckb-vm 将捕获这个系统调用, 然后将收到的字符串打印到标准输出. 因此, 您将看到:
...
故事并没有结束, 相反, 精彩的内容才刚刚开始! 我们将使用 AssemblyScripy 重写这个程序, 并演示如何在 AssemblyScript 代码中实现 syscall.

现在, 将目光聚焦在 `./wasc_dapp_demo_assemblyscript/assembly/index.ts` 文件上, 它由 AssemblyScript 编写, 逻辑和之前 C 写的代码差不多, 其源码为:
...
其中的 syscall 函数定义在 `env.ts` 文件中:

```ts
export declare function syscall(n: i64, a: i64, b: i64, c: i64, d: i64, e: i64, f: i64, mode: i64): i64
```

需要额外关注的是 syscall 的最后一个参数 mode, 它目前被传入了值 0b100000. syscall 的第一个参数是 `changetype<usize>(strEncoded)`, 从语义上来讲它是一个字符串的指针, 但存在问题, 因为这个字符串被定义在 WebAssembly 的内存中. mode 参数存在的意义便是告知我们的胶水代码, 这个传入的地址并非真实地址, 而是 WebAssembly 内存的偏移地址. 胶水代码会将通过该偏移地址取得真实的地址.

我们来编译这个项目, 它首先将生成 WebAssembly 输出, 然后经过 WASC 获得 RISC-V 输出. 最后在 ckb-vm 中执行它, 你的标准输出将同样获得相似的结果

```sh
$ cd wasc_dapp_demo_assemblyscript
$ npm run asbuild
